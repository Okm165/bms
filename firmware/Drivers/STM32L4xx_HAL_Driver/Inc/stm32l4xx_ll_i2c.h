...
typedef struct {
  ...
  uint32_t Timing; /*!< Specifies the SDA setup, hold time and the SCL high, low
                      period values. This parameter must be set by referring to
                      the STM32CubeMX Tool and the helper macro @ref
  ...
  uint32_t OwnAddress1; /*!< Specifies the device own address 1.
                             This parameter must be a value between Min_Data =
                           0x00 and Max_Data = 0x3FF.
  ...
  uint32_t TypeAcknowledge; /*!< Specifies the ACKnowledge or Non ACKnowledge
                               condition after the address receive match code or
                               next received byte. This parameter can be a value
                               of @ref I2C_LL_EC_I2C_ACKNOWLEDGE.

                                 This feature can be modified afterwards using
                               unitary function
                                 @ref LL_I2C_AcknowledgeNextData(). */

  uint32_t OwnAddrSize; /*!< Specifies the device own address 1 size (7-bit or
                           10-bit). This parameter can be a value of @ref
                           I2C_LL_EC_OWNADDRESS1.
...
#define LL_I2C_SMBUS_ALL_TIMEOUT                                               \
  (uint32_t)(I2C_TIMEOUTR_TIMOUTEN | I2C_TIMEOUTR_TEXTEN) /*!< TimeoutA and    \
TimeoutB (extended clock) enable bits */
/**
 * @}
 */
...
__STATIC_INLINE void LL_I2C_HandleTransfer(I2C_TypeDef *I2Cx,
  ...
  MODIFY_REG(
      ...
      SlaveAddr | SlaveAddrSize | (TransferSize << I2C_CR2_NBYTES_Pos) |
          EndMode | Request);
}
...
__STATIC_INLINE uint32_t LL_I2C_GetAddressMatchCode(I2C_TypeDef *I2Cx) {
  return (uint32_t)(READ_BIT(I2Cx->ISR, I2C_ISR_ADDCODE) >> I2C_ISR_ADDCODE_Pos
                                                                << 1);
}
