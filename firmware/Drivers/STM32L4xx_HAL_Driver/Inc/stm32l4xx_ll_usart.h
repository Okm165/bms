...
/**
  ...
  *         expected Baud Rate in 8 bits sampling mode (32 bits value of
...
/**
  ...
  *         @arg @ref LL_USART_PRESCALER_DIV256
  @endif
  * @param  __BAUDRATE__ Baud rate value to achieve
  * @retval USARTDIV value to be used for BRR register filling in
  OverSampling_16 case
  */
#if defined(USART_PRESC_PRESCALER)
 ...
 * @note   When this function is called, USART Clock is enabled while in STOP
 * mode
 ...
 * @note   When this function is called, USART Clock is disabled while in STOP
 * mode
 ...
 */
__STATIC_INLINE uint32_t
LL_USART_IsClockEnabledInStopMode(const USART_TypeDef *USARTx) {
...
/**
  ...
  *         (Baud rate value != 0)
  * @note   In case of oversampling by 16 and 8, BRR content must be greater
  than or equal to 16d.
...
/**
  * @brief  Return current Baud Rate value, according to USARTDIV present in BRR
  register
  *         (full BRR content), and to used Peripheral Clock and Oversampling
  mode values
...
LL_USART_IsActiveFlag_RXNE(const USART_TypeDef *USARTx) {
  return ((READ_BIT(USARTx->ISR, USART_ISR_RXNE) == (USART_ISR_RXNE)) ? 1UL
                                                                      : 0UL);
}
...
LL_USART_IsActiveFlag_TXE(const USART_TypeDef *USARTx) {
  return ((READ_BIT(USARTx->ISR, USART_ISR_TXE) == (USART_ISR_TXE)) ? 1UL
                                                                    : 0UL);
}
...
#endif /* USART_CR1_FIFOEN */
#if defined(USART_TCBGT_SUPPORT)
/* Function available only on devices supporting Transmit Complete before Guard
 * Time feature */
/**
...
__STATIC_INLINE uint32_t LL_USART_IsEnabledIT_TXE(const USART_TypeDef *USARTx) {
  return ((READ_BIT(USARTx->CR1, USART_CR1_TXEIE) == (USART_CR1_TXEIE)) ? 1U
                                                                        : 0U);
}
