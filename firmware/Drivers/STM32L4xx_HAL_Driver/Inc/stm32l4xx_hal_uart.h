...
typedef struct {
  uint32_t
      BaudRate; /*!< This member configures the UART communication baud rate.
                   ...
                   UART input clock (divided by a prescaler if applicable) UART:
                     =====
                   ...
                   ((huart->Init.BaudRate)))
                     - If oversampling is 8,
                        ...
                        Baud Rate Register[2:0] =  (((2 * uart_ker_ckpres) /
                                                    ((huart->Init.BaudRate)))[3:0])
                   >> 1 where uart_ker_ck_pres is the UART input clock (divided
                   by a prescaler if applicable) */
