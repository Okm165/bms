...
const uint16_t int_flags = EIE_PKTIE;

void ENC_IRQHandler() {
  //    if(__HAL_GPIO_EXTI_GET_IT(GPIO_PIN_2) != RESET) {

  __HAL_GPIO_EXTI_CLEAR_IT(ENC_INT_Pin);
  BaseType_t xHigherPriorityTaskWoken = pdFALSE;
  if (xTimerPendFunctionCallFromISR(prvReceivePacket, NULL, 0,
...
uint8_t enc28j60_init(uint8_t *macadr) {
  ...
  enc28j60_wcr(MACON3,
               MACON3_PADCFG0 | MACON3_PADCFG2 | // Enable padding and automatic
                                                 // vlan frames recognition
                   MACON3_TXCRCEN | MACON3_FRMLNEN |
                   MACON3_FULDPX); // Enable crc & frame len chk
  ...
  // enc28j60_bfs(EIE, EIE_INTIE | EIE_TXIE | EIE_PKTIE | EIE_TXERIE |
  // EIE_RXERIE);
  enc28j60_bfs(EIE, EIE_INTIE | int_flags);
  // Enable Rx packets
...
void prvReceivePacket(void *buf, uint32_t pktlen) {
  enc28j60_bfc(EIE, (~EIE_INTIE) & int_flags); // mask enc28j60 interrupts

  uint8_t eir_flags = enc28j60_rcr(EIR);

  while (eir_flags & EIR_PKTIF || eir_flags & EIR_TXIF ||
         eir_flags & EIR_TXERIF || eir_flags & EIR_RXERIF) {
  ...
  enc28j60_bfs(EIE, EIE_INTIE | int_flags); // unmask enc28j60 interrupts
}
