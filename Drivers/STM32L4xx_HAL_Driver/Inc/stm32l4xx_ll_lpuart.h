 ...
 * @note   When this function is called, LPUART Clock is enabled while in STOP
 * mode
 ...
 * @note   When this function is called, LPUART Clock is disabled while in STOP
 * mode
 ...
 */
__STATIC_INLINE uint32_t
LL_LPUART_IsClockEnabledInStopMode(const USART_TypeDef *LPUARTx) {
  return ((READ_BIT(LPUARTx->CR3, USART_CR3_UCESM) == (USART_CR3_UCESM)) ? 1UL
                                                                         : 0UL);
}
...
/**
  * @brief  Configure LPUART BRR register for achieving expected Baud Rate
  value.
  *
  ...
  * @note   Peripheral clock and Baud Rate values provided as function
  parameters should be valid
  *         (Baud rate value != 0).
  * @note   Provided that LPUARTx_BRR must be > = 0x300 and LPUART_BRR is
  20-bit,
  *         a care should be taken when generating high baud rates using high
  PeriphClk
  *         values. PeriphClk must be in the range [3 x BaudRate, 4096 x
  BaudRate].
...
/**
  * @brief  Return current Baud Rate value, according to LPUARTDIV present in
  BRR register
  ...
  if (lpuartdiv >= LPUART_BRR_MIN_VALUE) {
    ...
    brrresult =
        (uint32_t)(((uint64_t)(periphclkpresc)*LPUART_LPUARTDIV_FREQ_MUL) /
                   lpuartdiv);
#else
    brrresult = (uint32_t)(((uint64_t)(PeriphClk)*LPUART_LPUARTDIV_FREQ_MUL) /
                           lpuartdiv);
#endif /* USART_PRESC_PRESCALER */
  } else {
    brrresult = 0x0UL;
 ...
 */
__STATIC_INLINE uint32_t
LL_LPUART_IsActiveFlag_RXNE(const USART_TypeDef *LPUARTx) {
  return ((READ_BIT(LPUARTx->ISR, USART_ISR_RXNE) == (USART_ISR_RXNE)) ? 1UL
                                                                       : 0UL);
}
#endif /* USART_CR1_FIFOEN */
 ...
 */
__STATIC_INLINE uint32_t
LL_LPUART_IsActiveFlag_TXE(const USART_TypeDef *LPUARTx) {
  return ((READ_BIT(LPUARTx->ISR, USART_ISR_TXE) == (USART_ISR_TXE)) ? 1UL
                                                                     : 0UL);
}
#endif /* USART_CR1_FIFOEN */
 ...
 */
__STATIC_INLINE uint32_t
LL_LPUART_IsEnabledIT_RXNE(const USART_TypeDef *LPUARTx) {
 ...
 */
__STATIC_INLINE uint32_t
LL_LPUART_IsEnabledIT_TXE(const USART_TypeDef *LPUARTx) {
  return ((READ_BIT(LPUARTx->CR1, USART_CR1_TXEIE) == (USART_CR1_TXEIE)) ? 1UL
                                                                         : 0UL);
}
#endif /* USART_CR1_FIFOEN */
